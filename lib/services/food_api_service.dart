...
class FoodApiService {
  ...
  Food? _parseFood(Map<String, dynamic> product) {
    try {
      ...
      final calories = _parseDouble(nutriments['energy-kcal_100g']) ?? 
                     _parseDouble(nutriments['energy_100g'])?.let((energy) => energy * 0.239) ?? 0.0;
  ...
  String _determineCategory(String categories) {
    ...
    if (categories.contains('meat') || categories.contains('poultry')) return 'protein';
    if (categories.contains('dairy') || categories.contains('milk')) return 'dairy';
    if (categories.contains('cereal') || categories.contains('bread')) return 'grains';
    if (categories.contains('snack') || categories.contains('cookie')) return 'snacks';
    if (categories.contains('beverage') || categories.contains('drink')) return 'beverages';
    return 'general';
  }
